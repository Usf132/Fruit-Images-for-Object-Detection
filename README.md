...
**Detection data** (Roboflow):
* [`projects-gmksb/fruit-and-veggie`](https://universe.roboflow.com/projects-gmksb/fruit-and-veggie), version 9 — 11 classes, object type only, no ripeness variants.
* [`fruit-7ufam/dataset-official`](https://universe.roboflow.com/fruit-7ufam/dataset-official), version 18 — 30 classes, ripe/unripe variants of 15 fruits.

These two datasets are merged (see [How it was trained](#how-it-was-trained)) into the final 21-class detection set used to train `yolo11n`.

**Classification data** (Kaggle):
* [`utkarshsaxenadn/fruits-classification`](https://www.kaggle.com/datasets/utkarshsaxenadn/fruits-classification), used to train `yolo11n-cls`.

**Base models:**
* [Ultralytics YOLO11](https://github.com/ultralytics/ultralytics) (`yolo11n.pt`, `yolo11n-cls.pt`) — Ultralytics.

Thanks to the dataset authors above and to the Roboflow/Kaggle/Ultralytics communities for making these resources available.

> Note: dataset licenses aren't specified here — check each Roboflow/Kaggle source page for its license terms before redistributing the data itself (this repo only redistributes the *trained weights*, not the source images).
...
The trained model weights (`weights/best_detect.pt`, `weights/best_cls.pt`) are derived from the datasets listed above; their use is subject to those datasets' own license terms in addition to this repo's MIT license.
